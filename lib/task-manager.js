...
class TaskManager {
  ...
  createCR(from, reason, affectedTasks = [], opts = {}) {
    ...
    // Task 1: CR deduplication - check if similar CR already exists
    ...
    // Check existing CRs for duplicates
    let existingCRs;
    try {
      existingCRs = fs.readdirSync(crDir).filter(f => f.endsWith('.json'));
    } catch {
      existingCRs = [];
    }

    for (const crFile of existingCRs) {
      try {
        const existingCR = JSON.parse(fs.readFileSync(path.join(crDir, crFile), 'utf8'));
        if (existingCR.contentHash === contentHash && existingCR.status === 'pending') {
          console.log(`CR already exists: ${existingCR.id} (duplicate detected)`);
          return existingCR.id;
        }
      } catch {}
    }
