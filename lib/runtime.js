...
class Runtime {
  constructor(projectDir, opts) {
    ...
    // Usage: { type: "function", fn: "markMilestoneComplete" }
    this.functions = {
      markMilestoneComplete: () => this._markMilestoneComplete(),
  ...
  _markMilestoneComplete() {
    ...
    var data = this.getMilestones();
    var target = (data.milestones || []).find(m => m.id === msId);
    if (target) {
  ...
  _crossValidateMonitors() {
    ...
    if (msId) {
      ...
      if (!fs.existsSync(reviewDir)) fs.mkdirSync(reviewDir, { recursive: true });
      var summary = {
        milestoneId: msId, timestamp: new Date().toISOString(),
