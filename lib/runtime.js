...
class Runtime extends RuntimeInterface {
  constructor(projectDir, opts) {
    ...
    this.functions = {
      ...
      syncTeamState: () => this._syncTeamState(),
        ...
        if (code === 0) {
          ...
          if (shouldCommit) {
            try {
              ...
              // Scope git add by agent role — don't add -A (mixes everyone's changes)
              var gitAddPaths = {
                'developer': ['src/', 'lib/', 'test/', 'tests/', '__tests__/', 'package.json', 'tsconfig.json', '.team/tasks/', '.team/change-requests/'],
                'tester': ['test/', 'tests/', '__tests__/', '.team/tasks/'],
                'pm': ['.team/milestones/', '.team/tasks/', '.team/change-requests/', 'PRD.md'],
                'architect': ['ARCHITECTURE.md', '.team/change-requests/'],
                'tech_lead': ['.team/milestones/', '.team/tasks/'],
                'qa_lead': ['.team/milestones/', 'EXPECTED_DBB.md'],
                'vision_monitor': ['.team/gaps/vision.json'],
                'prd_monitor': ['.team/gaps/prd.json'],
                'dbb_monitor': ['.team/gaps/dbb.json', '.team/gaps/milestones/'],
                'arch_monitor': ['.team/gaps/architecture.json']
              };
              var paths = gitAddPaths[baseType] || ['.team/'];
              var addCmd = paths.map(function(p) { return 'git add ' + p + ' 2>/dev/null'; }).join('; ');
              execSync(addCmd + '; git diff --cached --quiet || git commit -m "' + prefix + ': ' + agentType + ' completed"', {
  ...
  async notify(title, message, event) {
    ...
    if (notifyConfig.openclaw && notifyConfig.openclaw.sessionKey) {
      try {
        ...
        var notifyMsg = '[Team: ' + title + '] ' + message;
    ...
    if (notifyConfig.macos) {
      try {
        execSync('osascript -e \'display notification "' + message + '" with title "Team: ' + title + '"\'');
      } catch {}
    }
  }
  ...
  _generateReport() {
    ...
    var lines = [];
    lines.push('# Team Report — ' + projectName);
    lines.push('');
  ...
  _syncTeamState() {
    ...
    try {
      execSync('git add .team/', { cwd: this.projectDir, stdio: 'ignore' });
      execSync('git diff --cached --quiet', { cwd: this.projectDir, stdio: 'ignore' });
      // no changes, skip
    } catch {
      try {
        const ts = new Date().toISOString().replace('T', ' ').replace(/\..+/, '');
        execSync(`git commit -m "team: sync state ${ts}"`, { cwd: this.projectDir, stdio: 'pipe' });
        execSync('git push', { cwd: this.projectDir, stdio: 'pipe' });
        this.log('workflow', 'sync', '✓ .team/ state synced to git');
      } catch (err) {
        this.log('error', 'sync', 'git sync failed: ' + err.message);
      }
    }
  }
